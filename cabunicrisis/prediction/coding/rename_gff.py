...
def rename_gff(union_input_path, blast_input_path, input_file, output_path,run_out):
    ...
    if run_out == 3:
        ...
        blast_input_file = blast_input_path+"/"+input_file+".fna_blast"
        #union_input_file = the specific union file that correlates to the input file (ex: CGT2049_union.fna/faa)
    ...
    if "known_unknown_gff" in os.listdir(output_path):    #if known_unknown_(faa or fna) already exists in output_folder/fna folder
